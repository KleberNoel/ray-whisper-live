...
class TestSileroVadDeployment:
    ...
    def test_has_speech_returns_true(self, mock_timestamps: MagicMock) -> None:
        mock_timestamps.return_value = [{"start": 0, "end": 1024}]
        v = self._make_vad()

        result = v.has_speech(np.zeros(2048, dtype=np.float32))

        assert result is True
        mock_timestamps.assert_called_once()

    @patch("src.vad.get_speech_timestamps")
    def test_has_speech_returns_false(self, mock_timestamps: MagicMock) -> None:
    ...
    @patch("src.vad.get_speech_timestamps")
    def test_has_speech_short_audio_returns_true(
        self, mock_timestamps: MagicMock
    ) -> None:
        """Audio shorter than 512 samples should always return True."""
        v = self._make_vad()

        result = v.has_speech(np.zeros(256, dtype=np.float32))

        assert result is True
        mock_timestamps.assert_not_called()

    @patch("src.vad.get_speech_timestamps")
    def test_has_speech_passes_vad_options(self, mock_timestamps: MagicMock) -> None:
    ...
    @patch("src.vad.get_speech_timestamps")
    def test_get_speech_segments(self, mock_timestamps: MagicMock) -> None:
        expected = [{"start": 100, "end": 500}, {"start": 800, "end": 1200}]
        mock_timestamps.return_value = expected
        v = self._make_vad()

        result = v.get_speech_segments(np.zeros(2048, dtype=np.float32))

        assert result == expected

    @patch("src.vad.get_speech_timestamps")
    def test_get_speech_segments_empty(self, mock_timestamps: MagicMock) -> None:
        mock_timestamps.return_value = []
        v = self._make_vad()
